...
contract ConstructorTest is BaseConstructorTest {
    ...
    function test_constructor_RevertWhen_ZeroModuleAddress() public {
        ...
        accounting = new Accounting(address(locator), address(0), address(feeDistributor), 4 weeks, 365 days);
    }
    ...
    function test_constructor_RevertWhen_ZeroFeeDistributorAddress() public {
        ...
        accounting = new Accounting(address(locator), address(stakingModule), address(0), 4 weeks, 365 days);
    }

    function test_constructor_RevertWhen_InvalidBondLockPeriod_MinMoreThanMax() public {
        vm.expectRevert(IBondLock.InvalidBondLockPeriod.selector);
        accounting = new Accounting(address(locator), address(0), address(feeDistributor), 4 weeks, 2 weeks);
    }

    function test_constructor_RevertWhen_InvalidBondLockPeriod_MaxTooBig() public {
        vm.expectRevert(IBondLock.InvalidBondLockPeriod.selector);
        accounting = new Accounting(
            address(locator),
            address(0),
    ...
    function test_constructor_RevertWhen_InvalidBondLockPeriod_MinIsZero() public {
        vm.expectRevert(IBondLock.InvalidBondLockPeriod.selector);
        accounting = new Accounting(address(locator), address(0), address(feeDistributor), 0, 154 days);
    }
}
...
contract InitTest is BaseInitTest {
    ...
    function test_initialize_RevertWhen_zeroAdmin() public {
        ...
        accounting.initialize(curve, address(0), 8 weeks, testChargePenaltyRecipient);
    }
