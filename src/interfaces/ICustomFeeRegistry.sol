 ...
 *
 *    Z is the discount-cut cooldown. Off-chain reward distribution is expected to
 *    snapshot getEffectiveFee at the report refSlot and use that snapshot for the
 *    corresponding frame. Under this convention, keeping
 ...
 */
/// @notice Per-operator fee discounts and the allocation weight boost derived from them: the larger the
///         discount from BASE_FEE, the higher the weight. `Step.threshold` is a discount, `Step.value` the
///         weight multiplier increment above MAX_BP. The effective fee drives off-chain reward
///         distribution, whose timing this registry does not enforce. See the diagrams above.
interface ICustomFeeRegistry is IStepwiseWeightBoost {
    ...
    /// @notice Fee kept with no discount, in basis points. A positive fee modifier can lift the effective
    ///         fee above it.
    ...
    /// @param defaultMaxFeeDiscount Initial ceiling: a non-zero multiple of FEE_GRANULARITY below BASE_FEE.
    /// @param feeDiscountCutCooldown Duration in seconds, in [1, MAX_FEE_DISCOUNT_CUT_COOLDOWN].
    /// @param steps Initial steps: FEE_GRANULARITY-aligned thresholds below BASE_FEE, values up to
    ///        MAX_STEP_VALUE.
    function initialize(
        address admin,
    ...
    /// @notice Request a fee discount. Only the Node Operator owner. Raising it applies at once; lowering
    ///         schedules a cut for `applyFeeDiscountCut`, while the allocation weight follows the request
    ///         at once. Requesting the current discount reverts.
    ...
    /// @notice Cancel a pending discount cut, returning the allocation weight to the stored discount.
    ///         Only the Node Operator owner.
    /// @param nodeOperatorId ID of the Node Operator.
    ...
    /// @notice Set the fee modifier of an existing Node Operator type (bond curve). Only DEFAULT_ADMIN_ROLE.
    ///         Shifts the effective fee only, never the weight; a negative modifier lowers the type's ceiling.
    ...
    /// @notice Stored discount of the Node Operator; zero if never set, and unaffected by a pending cut.
    /// @param nodeOperatorId ID of the Node Operator.
    function getFeeDiscount(uint256 nodeOperatorId) external view returns (uint256);

    /// @notice Pending cut target. Meaningful only while a cooldown is active, since zero is a valid target.
    /// @param nodeOperatorId ID of the Node Operator.
    ...
    /// @notice Discount ceiling of the Node Operator: the default ceiling less the type's negative modifier.
    /// @param nodeOperatorId ID of the Node Operator.
    function getMaxFeeDiscount(uint256 nodeOperatorId) external view returns (uint256);

    /// @notice Effective fee in basis points: the stored discount adjusted by the fee modifier, clamped at
    ///         zero. Excludes a pending cut. Exposed for off-chain reward distribution.
    /// @param nodeOperatorId ID of the Node Operator.
